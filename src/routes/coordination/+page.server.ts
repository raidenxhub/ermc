...
export const load: PageServerLoad = async ({ locals: { supabase, user } }) => {
	...
	try {
		...
		const cid = myProfile?.cid ? String(myProfile.cid) : '';
		if (!cid) {
			return { user, access: false, reason: 'Missing VATSIM CID on your profile. Please set it in Settings.' };
		}

		const slurperUrl = new URL('https://slurper.vatsim.net/users/info');
		slurperUrl.searchParams.set('cid', cid);
